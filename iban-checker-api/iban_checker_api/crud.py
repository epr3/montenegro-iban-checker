...
def get_validations(db: Session, session_id: str, skip: int = 0, limit: int = 12):
  return db.query(models.Validation).filter(models.Validation.session_id == session_id).offset(skip).limit(limit).all()
